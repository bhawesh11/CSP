...
public class STC_Login {
	...
	public void login(Testing test) {
		...
		Login login = (Login) PageFactory.initElements(test.driver,
				test.getPage());
		test.webFunctions().type(test, login.textBox_Email,
				test.getOutput("Email"));
		test.webFunctions().type(test, login.textBox_Password,
				test.getOutput("Password"));
